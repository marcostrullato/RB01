...
class RoomberryHandler(BaseHTTPRequestHandler):
    ...
    def get_media_xml(self):
        ...
        media_dictionary = {}
        for dirpath, dirs, files in walk(PATH_TO_WWW + PATH_TO_MEDIA + PATH_TO_CAM):            
            if files:
        ...
        try:
            ...
            if self.path.startswith(PATH_TO_CAM):
                ...
                if self.path == PATH_TO_CAM_XML:
                    ...
                    self.send_reply(PATH_TO_WWW + PATH_TO_MEDIA,'image/jpg')
                elif self.path.endswith('.h264'):
                    self.send_reply(PATH_TO_WWW + PATH_TO_MEDIA,'video/H264')
                else:
